...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        *   어플리케이션이 실행될 때 어떤 클래스가 최초한 번만 메모리를 할당하고
        *   그 떄 메모리에 인스턴스를 만들어 하나의 인스턴스를 공휴해 사용하며
        *   메모리 낭비를 방지할 수 있게 함(매번 인스턴스를 생성하지 않음)
        *   장점
        *   1. 첫번째 이용 시에는 인스턴스를생성해야 하므로 속도 차이가 나지 않지만
...
//        EagerSingleton eager = new EagerSingleton();  생성자가 private이라 접근 불가
        ...
        /* 필기.
        *   이른 초기화를 사용하면 처음 인스턴스 반환요청에서 속도가 빠르다
