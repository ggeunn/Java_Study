...
//        int result1 = num1 + num2;    //자동으로 큰 쪽 자료형인 long으로 변경 후 계산하기 떄문에 int형 변수에 값을 담을 수 없다.
        long result1 = num1 + num2;     //int + long은 서로 다른 자료형이라 데이터 손실이 발생하지 않는 int -> long 변환을 자동으로 수정한다.
        ...
        /* 필기.
        *   정수를 실수로 변경할 때 소수점 자리수가 없어도 실수 형태로 표현이 가능하다. 이 떄 데이터 손실이 없기 때문에 자동으로 진행한다. */
